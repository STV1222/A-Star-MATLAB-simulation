function A_star_pathfinding
    ...
    obstaclesrec = {[50,50,75,25],[150,180,40,120],[194,100,10,150],[350,240,75,210],[205,1,80,249],[70,360,150,50],[280,330,30,170]};
...
function [parents, searchArea] = aStarSearch(startX, startY, goalX, goalY, MAX_COLS, MAX_ROWS, TOTAL_NODES, LARGE_VALUE, obstaclesrec)
    ...
    while true
        ...
        neighborOffsets = [0 1; 0 -1; -1 0; 1 0]; % Up, Down, Left, Right
        for i = 1:size(neighborOffsets, 1)
            dx = neighborOffsets(i, 1);
            dy = neighborOffsets(i, 2);
            ...
            if ~isValidPosition(newX, newY, MAX_COLS, MAX_ROWS, obstaclesrec)
                continue; % Skip if not a valid position
            end
            ...
            tentativeG = g(currentIdx) + 1; % Orthogonal move cost is 1

            if ~openList(neighborIdx) || tentativeG < g(neighborIdx)
                parents(neighborIdx) = currentIdx;
                g(neighborIdx) = tentativeG;
                h(neighborIdx) = abs(newX - goalX) + abs(newY - goalY); % Manhattan distance
...
function plotEnvironmentAndPath(parents, searchArea, startX, startY, goalX, goalY, MAX_COLS, obstaclesrec, objectSize)
    ...
    title('A* Pathfinding - Manhattan distance'); xlabel('X'); ylabel('Y');
    ...
    % Plot path
    plot(pathX, pathY, 'b-', 'LineWidth', 5);
